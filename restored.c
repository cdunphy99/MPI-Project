...
void  my_mm6_sse2_mpi(int localN, int localK, int N, int M, int K, double *a, double *b, double *c)
{
  ...
  for (g=0; g < world_size; g++){
    for (i=0; i < localK; i++){
      for (j=0; j < localN; j++){
        for (k=0; k < M; k++){
          *(c+ pRank*localK*localN + i*localN + j) = *(c+ pRank*localK*localN + i*localN + j) + *(a+j*M + k)*  *(b + k*localK + i);
    ...
    if (curB == 0) { // first
      ...
      curB = world_size - 1;
    } else if (curB == (world_size - 1)) { // last
      printf("prank world_size - 1 running\n");
      MPI_Send(b,1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
      MPI_Recv(b,1, MPI_DOUBLE, curB - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      curB--;
    } else { // in between
      printf("prank in between running\n");
      MPI_Send(b, 1, MPI_DOUBLE, curB + 1, 0, MPI_COMM_WORLD);
      MPI_Recv(b, 1, MPI_DOUBLE, curB - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      curB--;
    }
    MPI_Barrier(MPI_COMM_WORLD);
